...
// part 1
	foreach($aInstructions as $instruction) {
		$instruction = str_replace('turn ', '', $instruction);
		$instruction = str_replace('through ', '', $instruction);
		list($step, $start, $end) = explode(' ', $instruction);
	...
	function dotoggle($x, $y) {
		global $coordArr;

		$coordArr['x'.$x.'y'.$y] = ($coordArr['x'.$x.'y'.$y] == 1) ? 0 : 1;
	}
